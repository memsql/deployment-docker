...
${AWS_CMD} s3 cp "${RESULT_DIR}/${CLUSTER_NAME}.tar.gz" "s3://${S3_REPORT_BUCKET}/${S3_REPORT_PATH}/report.tar.gz"

cat <<EOF > "${RESULT_DIR}/metadata.txt"
cluster_name: "${CLUSTER_NAME}"
created_at: "$(date)"
EOF
${AWS_CMD} s3 cp "${RESULT_DIR}/metadata.txt" "s3://${S3_REPORT_BUCKET}/${S3_REPORT_PATH}/metadata.txt"
...
if [[ -n "${OPERATOR_POD}" ]]; then
    ...
    ${AWS_CMD} s3 cp "${OPERATOR_LOG}" "s3://${S3_REPORT_BUCKET}/${S3_REPORT_PATH}/operator.log"
else
...
${AWS_CMD} s3 cp "${RESULT_DIR}/${CLUSTER_NAME}.txt" "s3://${S3_REPORT_BUCKET}/${S3_REPORT_PATH}/check.txt"
...
then
    ...
    ${AWS_CMD} s3 cp "${RESULT_DIR}/noCollect.txt" "s3://${S3_REPORT_BUCKET}/${S3_REPORT_PATH}/noCollect.txt"
    exit 1
fi
