...
# the memsql user should be 999, change any user that was 999 to something different
# in alma this is currently systemd-coredump
user999=$(id -nu 999)
RESULT_USER=$?

# the memsql user group should be 998, change any groupid that was 998 to something different
...
if [[ "$RESULT_USER" -eq 0 ]]; then
  ...
  if [[ ! -z "$user_files" ]]; then
    echo "found files of user ${user999}, cannot create memsql user"
    exit 1
  fi
...
if [[ "$RESULT_GROUP" -eq 0 ]]; then
  ...
  if [[ ! -z "$group_files" ]]; then
    echo "found files of group ${group998}, cannot create memsql user"
    exit 1
  fi
...
groupadd -g 998 memsql
useradd memsql -u 999 -g 998
